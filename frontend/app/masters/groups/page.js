  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <input {...register("name")} className="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Sharma Family" />
      ...
      <div>
        ...
        <select {...register("status")} className="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-2.5 text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
...
const columns = [
  { accessorKey: "name", header: "Group Name", cell: ({ getValue }) => <span className="font-medium text-slate-200">{getValue()}</span> },
...
export default function GroupsPage() {
  return (
    <CrudPage
      title="Groups"
      description="Manage investor groups and families"
