...
function CompanyForm({ data, onSubmit, isLoading, onCancel }) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    ...
    defaultValues: { name: "", symbol: "", sector: "", industry: "", status: "Active" },
  });

  useEffect(() => {
    if (data) reset({ name: data.name, symbol: data.symbol, sector: data.sector || "", industry: data.industry || "", status: data.status });
    else reset({ name: "", symbol: "", sector: "", industry: "", status: "Active" });
  }, [data, reset]);
